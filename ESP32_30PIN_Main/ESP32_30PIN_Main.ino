...
const float BOWL_MISSING_THRESHOLD = -15.0;  
const int DRINK_DETECT_THRESHOLD = 5;        
...
// 🌟 ย้าย Validate EEPROM มาไว้ตรงนี้ (หลังสร้าง Struct เสร็จแล้ว)
static_assert(ADDR_SCHEDULES + (sizeof(FeedingTime) * 3) <= EEPROM_SIZE, "ERROR: EEPROM_SIZE too small!");
...
void startFeeding(int value, FeedMode mode, unsigned long now) {
  ...
  if (currentWeight >= limitBowlFood) return;
  ...
  if (currentWeight >= feedTargetWeight) return;
  ...
  switch (feedState) {
    case FORWARD:
      ...
      if (currentWeight >= feedTargetWeight) {
        ...
        } else if (now - targetReachedTime > 1000) {  
          feedServo.write(SPEED_STOP);
          stopTimer = now;  
          feedState = FINISH;
          targetReachedTime = 0;
...
void processWater(unsigned long now) {
  ...
  switch (waterState) {
    ...
    case WATER_WAITING:
      float bowlSensorValue = LoadCell_BowlWater.getData();  

      if (bowlSensorValue < BOWL_MISSING_THRESHOLD) {  
        waterWaitTimer = now;
      } else if (bowlWater < lastDrinkWeight - DRINK_DETECT_THRESHOLD) {  
        waterWaitTimer = now;
        lastDrinkWeight = bowlWater;
    ...
    case WATER_PUMPING:
      if (bowlWater >= (limitBowlWater - 40)) {
        ...
        switch (currentPage) {
          ...
          case 2:
            if (confirmMode == 1) drawConfirmPage("Confirm Reset", "WiFi?", 1);
            else if (confirmMode == 2) drawConfirmPage("Confirm Delete", "Round: " + String(deleteIdx + 1), 2);
            break;
...
void drawHomePage() {
  currentPage = 0; tft.fillScreen(TFT_BLACK);
  tft.fillRect(0, 0, 320, 40, TFT_NAVY); tft.setTextColor(TFT_WHITE, TFT_NAVY); tft.setTextSize(2); tft.setCursor(10, 10); tft.print("Smart Pet Feeder");
  tft.drawRect(10, 50, 300, 30, TFT_WHITE);
  tft.fillRect(15, 90, 140, 40, TFT_BABYBLUE); tft.drawRect(15, 90, 140, 40, TFT_WHITE); tft.setTextColor(TFT_BLACK, TFT_BABYBLUE); tft.setCursor(31, 102); tft.print("Food:"); tft.print(tankFood); tft.print("%");
  tft.fillRect(15, 140, 140, 40, TFT_BABYBLUE); tft.drawRect(15, 140, 140, 40, TFT_WHITE); tft.setTextColor(TFT_BLACK, TFT_BABYBLUE); tft.setCursor(20, 152); tft.print("Bowl:"); tft.print(bowlFood); tft.print("g.");
  tft.fillRect(15, 190, 140, 40, TFT_DARKGREEN); tft.drawRect(15, 190, 140, 40, TFT_WHITE); tft.setTextColor(TFT_WHITE, TFT_DARKGREEN); tft.setCursor(37, 202); tft.print("FEEDFOOD");
  tft.fillRect(165, 90, 140, 40, TFT_CREAMYELLOW); tft.drawRect(165, 90, 140, 40, TFT_WHITE); tft.setTextColor(TFT_BLACK, TFT_CREAMYELLOW); tft.setCursor(175, 102); tft.print("Water:"); tft.print(tankWater); tft.print("%");
  tft.fillRect(165, 140, 140, 40, TFT_CREAMYELLOW); tft.drawRect(165, 140, 140, 40, TFT_WHITE); tft.setTextColor(TFT_BLACK, TFT_CREAMYELLOW); tft.setCursor(170, 152); tft.print("Bowl:"); tft.print(bowlWater); tft.print("mL");
  tft.fillRect(165, 190, 140, 40, TFT_BLUE); tft.drawRect(165, 190, 140, 40, TFT_WHITE); tft.setTextColor(TFT_WHITE, TFT_BLUE); tft.setCursor(211, 202); tft.print("MENU");
  lastTimeStr = ""; lastWifiStatus = !wifiStatus; forceUpdateTank = true;
}

void drawMenuPage() {
  currentPage = 1; tft.fillScreen(TFT_BLACK); tft.setTextSize(2);
    ...
    switch (actionButton) {
      ...
      case 20:
        if (confirmMode == 1) { resetWiFi(); } 
          ...
          if (WiFi.status() == WL_CONNECTED) {
            ...
            char json[256]; snprintf(json, sizeof(json), "{\"type\":\"delete_schedule_from_esp\",\"token\":\"%s\",\"time\":\"%s\"}", myToken, timeStr);
            client.send(json);
          }
      ...
      case 48:
        ...
        if (WiFi.status() == WL_CONNECTED) {
          ...
          char json[256]; snprintf(json, sizeof(json), "{\"type\":\"add_schedule_from_esp\",\"token\":\"%s\",\"time\":\"%s\",\"duration\":%d,\"slot\":%d}", myToken, timeStr, schedules[editIdx].gram, editIdx + 1);
          client.send(json);
        }
        drawMenuPage(); break;
...
void setup() {
  ...
  // เริ่มต้น Webserver หรือกระบวนการ WiFi ที่นี้ (ฟังก์ชัน startWifi ต้องถูกเรียกถ้ามี)
  // startWifi(); 
  // เนื่องจากในโค้ดเก่าส่วน startWifi หลุดหายไปตอนเกิด Git Conflict ผมเตรียมให้ด้านล่าง
